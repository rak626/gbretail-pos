      ...
      <TableCell className="w-36">
        {isLoose && !isCustom ? (
          <div className="flex items-center justify-center gap-2">
            <Button variant="outline" size="icon" className="h-11 w-11 rounded-full p-0 shrink-0" onClick={() => onLooseEdit(index)} aria-label="Edit loose weight">
              <Minus className="w-4 h-4" />
            </Button>
            <span onClick={() => onLooseEdit(index)} className="min-w-[72px] h-11 flex items-center justify-center rounded-full bg-primary/10 border border-primary/20 text-primary text-[13px] font-bold tabular-nums cursor-pointer hover:bg-primary/15 px-2" title="Tap to edit weight">
              {formatLooseQty(item.weight)}
            </span>
            <Button size="icon" className="h-11 w-11 rounded-full p-0 shrink-0 bg-primary hover:bg-primary/90 text-primary-foreground" onClick={() => onLooseEdit(index)} aria-label="Edit loose weight">
          ...
          <div className="flex items-center justify-center gap-2">
            <Button variant="outline" size="icon" className="h-11 w-11 rounded-full p-0 shrink-0" onClick={() => onUpdateQty(index, Math.max(0, (item.quantity || 0) - 1))} aria-label="Decrease quantity">
              <Minus className="w-4 h-4" />
            </Button>
            <span className="min-w-[40px] h-11 flex items-center justify-center rounded-full bg-muted border text-[15px] font-bold tabular-nums">
              {item.quantity || 0}
            </span>
            <Button size="icon" className="h-11 w-11 rounded-full p-0 shrink-0 bg-primary hover:bg-primary/90" onClick={() => onUpdateQty(index, (item.quantity || 0) + 1)} aria-label="Increase quantity">
              <Plus className="w-4 h-4" />
            </Button>
          </div>
        )}
