...
type UserRow = { id: string; email: string; name: string; role: string; shopId: string | null; isActive: boolean; shop?: { id: string; name: string } | null; createdAt?: string };
...
export default function UsersPage() {
  ...
  const [newUser, setNewUser] = useState({ email: "", password: "", name: "", role: "STAFF", shopId: "" });
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setShowForm(false);
      setNewUser((s) => ({ ...s, email: "", password: "", name: "" }));
      await load();
    } catch (e) {
            ...
            <CardContent className="p-0">
              <div className="overflow-auto max-h-[60vh] border-t">
                <table className="w-full text-xs">
                  <thead className="bg-muted/30 sticky top-0"><tr><th className="text-left p-2">Name</th><th className="text-left p-2">Email</th><th className="text-left p-2">Role</th><th className="text-left p-2">Shop</th><th className="text-right p-2">Active</th><th className="text-right p-2">Action</th></tr></thead>
                  <tbody>
                    {users.map((u) => (
                      <tr key={u.id} className="border-t">
                        ...
                        <td className="p-2 text-right">{u.isActive ? "Yes" : "No"}</td>
                        <td className="p-2 text-right">
                          <Button variant="ghost" size="sm" className="h-6 w-6 p-0" onClick={async () => { if (!confirm(`Delete user ${u.name}? Soft delete.`)) return; try { await apiClient.delete(`/api/users/${u.id}`); await load(); } catch (e) { setError(e instanceof Error ? e.message : "Failed"); } }} disabled={u.id === user.id}>
                            <Trash2 className="w-3 h-3" />
                          </Button>
                        </td>
                      </tr>
                    ))}
                    {users.length === 0 && !loading && <tr><td colSpan={6} className="p-6 text-center text-sm text-muted-foreground">No users found</td></tr>}
                  </tbody>
                </table>
              </div>
            </CardContent>
